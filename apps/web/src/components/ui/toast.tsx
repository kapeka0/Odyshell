  ...
  return (
    <ToastPrimitive.Root
      ...
      className={cn(
        ...
        "h-(--height) [transform:translateX(var(--toast-swipe-movement-x))_translateY(calc(var(--toast-swipe-movement-y)-(var(--toast-index)*var(--peek))-(var(--shrink)*var(--height))))_scale(var(--scale))] [transition:transform_420ms_var(--ease-out),opacity_250ms,height_150ms]",
...
function ToastIcon({ type }: { type: string | undefined }) {
  ...
  return (
    ...
    >
      <Icon aria-hidden="true" className={cn(type === "loading" && "animate-spin")} />
    </span>
  );
}
