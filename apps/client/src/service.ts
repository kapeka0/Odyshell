...
export async function installClientService(
  ...
  return await installLinuxUserService(options);
}
...
export async function removeClientService(
  ...
  await removeLinuxUserService(configPath);
}
...
export async function restartClientService(
  ...
  assertLinuxSystemd();
  await activateLinuxUserService(linuxServiceNameForConfig(configPath));
}
