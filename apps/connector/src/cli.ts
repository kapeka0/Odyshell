...
import { enrollConnector, runConnector } from "./index.js";
...
if (command === "enroll") {
  const result = await enrollConnector({
    ...
    configPath: option("config", ".odyshell/connector.json")!,
    image: option("image", "alpine:3.22") ?? "alpine:3.22",
  });
  console.log(JSON.stringify({ enrolled: true, ...result }, null, 2));
} else if (command === "start") {
  await runConnector(option("config", ".odyshell/connector.json")!);
} else {
  console.error("Usage: ods-connector <enroll|start> [options]");
  process.exitCode = 1;
}
