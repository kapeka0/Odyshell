...
import type { ApprovedMcpRuntime, ApprovedMcpOperationResult } from "@odyshell/mcp";
import {
...
registerRemoteMcp(app, process.env, {
  database: db,
  runtime: (installation) => createRemoteMcpRuntime(installation),
});

function createRemoteMcpRuntime(
  ...
  return {
    ...
    async ping(machineReference) {
      const machine = await resolveMcpMachine(installation, machineReference);
    ...
    async request(input) {
      ...
      const machine = await resolveMcpMachine(installation, input.machine);
      let scope;
      try {
        scope = operationSessionScope(machine.id, input.action);
      } catch {
        throw new RemoteMcpError(
    ...
    async status(requestId) {
      ...
      if (granted) return remoteMcpSessionStatus(granted);
      if (bound) {
        return {
      ...
      return remoteMcpSessionStatus(principal);
    },
    async execute(input) {
      const principal = await db.findMcpSessionPrincipal({
        workspaceId: installation.workspaceId,
