...
describe("Session-native authority cutover", () => {
  ...
  it("accepts only Session protocol data after Machine authentication", () => {
    ...
    expect(protocol).toContain("sessionProfile: {");
    expect(protocol).not.toContain('type: "task.open"');
    expect(protocol).not.toContain('type: "operation.start"');
    expect(gateway).not.toContain('case "task.opened"');
